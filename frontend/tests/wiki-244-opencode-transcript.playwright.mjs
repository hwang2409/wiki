...
async function main() {
  ...
  try {
    ...
    page.on("response", (response) => {
      ...
      const cursor = Number(new URL(url).searchParams.get("cursor") ?? "-1");
      subagentCalls.push(
        response.text().then((body) => ({ cursor, bytes: body.length, events: (() => {
          try { return JSON.parse(body).events?.length ?? -1; } catch { return -1; }
        })() })).catch(() => ({ cursor, bytes: -1, events: -1 })),
      );
    });
    ...
    const insertGeom = await page.evaluate(() => {
