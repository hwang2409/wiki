...
- **GitHub**: **App-credentialed read lane DONE** — creds `ADMIN_AGENT_GITHUB_APP_*` in prod, installation-token minting, App-native repo visibility + `dynamic_visibility` readiness (PHO-12666/#10238); tools: `list_admin_github_repositories`, `search_admin_github_pull_requests`, `inspect_admin_github_pull_request` (+diff), `get_admin_github_file`. **Sub-task B MERGED (#10622, 07-06):** `POST /webhooks/github/admin-agent`, fail-closed `ADMIN_AGENT_GITHUB_WEBHOOK_SECRET` verification, normalized `admin_agent_github_webhook_events` persistence, and route tests for signature/dedupe/event-family/unknown-event handling. **Phase 1 split MERGED (PHO-13093/#10690, 07-07, 12fa6c0c77):** all admin code now lives in `libraries/python/phoebe_admin_agent/` (202 files moved, pure extraction, zero behavior change). Dependency is one-way (admin → event_agent mechanics, barrel-only). Shared `runtime_assembly.py` builds admin runs via an injection seam — `phoebe_admin_agent` must be imported before building an admin run (named-dependency guard raises clearly if not; the three prod callers import eagerly). Phase 2 rescoped 07-07: admin-shaped run tables in the SAME DB, not a separate instance — see [[admin-agent-phase2-tables-not-db]].

**Webhook activation (07-06):** secret set in prod `phoebe-app-env-vars` (`ADMIN_AGENT_GITHUB_WEBHOOK_SECRET`), App `phoebe-ci` webhook configured + 7 events subscribed via UI (hook-config API 404s until first UI setup); deliveries 401 until next prod API deploy loads the secret — redeliver from App Recent Deliveries after. **Leg C MERGED (PHO-13111/#10651, 07-06):** PR review pipeline — `review PR <url>` trigger, bounded review subagent, scoped write token (pull_requests:write only, payload-asserted tests), head-SHA discipline + push dedupe, audit row per write, repo allowlist fail-closed, live-mode guard, write-lane source health. **Remaining (PHO-12306):** D PR creation, E write gates, F UX. In-flight nearby: #10552 (woodbridge) adds repo primitives + deploy doctrine skill — blocked on missing `actions: read` App permission.
...
- **Subagent inheritance hardening (PHO-13227, PR #10861)**: child runs now snapshot the parent `ToolApprovalConfig` into `run_metadata.subagent.tool_approval`, rehydrate it during bundle build, and intersect it with the child's actual mounted tool names so children never widen approvals. Any tool whose `CapabilityManifest.approval_requirements` includes `TOOL_APPROVAL` or `OWNER_SIGNOFF` is structurally unmounted in subagents unless `run_metadata.subagent.pre_approved_tool_calls` contains an exact `{tool_name, args_fingerprint, expires_at}` match; direct execution then returns a synthetic permission error instead of silently widening the surface. This mount-time wrapper is meant to stay effective even after PHO-13218 pack-driven mounts.
...
- **Economics**: summary-first tiering — compact model-visible output, full payloads to `admin_artifacts`, `inspect_*` drill-down (PHO-12938/#10448); cache-friendly stable prompt prefix (PHO-11689/12831). Durable admin-agent artifacts live in `app.admin_artifacts` / `AdminArtifact` (see `libraries/python/phoebe_event_agent/admin_artifacts.py`, `services/api/routes/admin/agent/agent.py`, `database/schema.sql` around `CREATE TABLE admin_artifacts`), not in `app.phoebe_agent_run_resources`; that table is for run↔resource bindings like outreach/resource association. Lesson: cache hit rate, not model tier, is the cost lever; 5-min cache TTL expiries rewrote ~260k tokens each in the $9.98 run.
