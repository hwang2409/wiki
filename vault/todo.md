...
- [P2] WIKI-102: knowledge index diet — 1.9G db from 650k event chunks storing 1.27GB FULL event text in chunks table (chunks=1385MB vs chunks_fts=301MB; notes ~0MB). Text duplicates events.jsonl on disk; citations are run_id+seq so full text is fetchable from source. Fix: (1) clip event chunk text to excerpt (~2KB) — FTS snippets unaffected; (2) skip low-value events at ingest (tool outputs > N KB, base64 blobs, ANSI-heavy legacy pane-log lines); (3) legacy ~/me/fun/agent-archive sweep behind opt-in flag (biggest contributor). Target ~400M total, no query-latency regression (search touches 301MB FTS index, already fine). Bump schema_version -> drop+rebuild migrates. Baseline numbers from first live rebuild 2026-07-14.
