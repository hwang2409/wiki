...
- **WIKI-130 DEPLOYED 2026-07-15 ~21:10**: launch regression fixed (1b1cacb — WIKI-129 baked staging path into compile-time `resolve_repo_dir()`; now `.native-build-staging` ancestor marker like `.codex`, first Rust unit tests in src-tauri). Supervisor SIGTERM → atomic swap → relaunch verified. Fleet auto-resumed via `recover_on_start` (5 codex + 3 claude sessions re-parented). Stacked backend changes NOW LIVE: 127 workspace endpoints, 116 diagnostic dedupe, 129 app.lock guard. Rebuild debt cleared. Backend changes stacked awaiting native rebuild: 127 workspace endpoints, 116 diagnostic dedupe (store.py), 129 app.lock acquisition (guard protection only fully active once rebuilt GUI/sidecar deployed). New build flow post-129: staged + atomic, refuses under running app, `FORCE_STAGE_ONLY=1` to prepare while app open. Frontend dist current (rebuilt after each merge).
...
- Safe deploy flow proven: SIGTERM supervisor (graceful drain) → swap-native-app.sh → relaunch; fleet exact-session-resumes. Supervisor parents all workers/orchestrators — never kill -9.
