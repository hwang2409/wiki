...
- **WIKI (orch `wiki-dev`)**: WIKI-244 MERGED (#178, 2026-08-06). 2026-08-05 hotfixes landed direct to main (3c180a3 orphan-sweep once-per-boot, a0b4f0c agent-prose monospace, c1c86e2 build-script venv python). NEW spawn wave 2026-08-06: cdx luna implementers on **WIKI-245** (transcript readability redesign — Henry TOP priority: unit grouping, chrome diet, semantic labels, harness-wrapper stripping; screenshots /tmp/WIKI-245-example-{1,2}.png), **WIKI-242** (a11y final pass, scope-guarded off transcript restructuring to avoid WIKI-245 conflicts), **WIKI-227** (incremental costs scan + slim /api/agents); fleet monitor armed off `.wiki-dev-watchlist`; boot-sweep follow-up (terminal-run skip + store pruning) filed as P2 todo.
