...
- **WIKI (orch `wiki-dev`)**: **Screenshot bug-fix arc COMPLETE 2026-08-06 eve — WIKI-258..261 all merged, gate-only (no reviewers), cc opus-4.7 implementers**: #194 WIKI-259 (empty events no longer reserve virtual row height), #195 WIKI-258 (split-diff 1ch column collapse), #196 WIKI-260 (font picker enumerates installed families via new backend `/api/fonts`; no variant grouping — JetBrains Mono ≠ Nerd Font; endpoint live only after next app rebuild), #197 WIKI-261 (numbered read-tool outputs highlighted: gutter stripped, tokenized, gutter column; needed a rebase steer — conflicted with the other three merges). Fleet EMPTY, todos pruned, done.md logged. **Henry should rebuild + relaunch Wiki.app to pick up all four fixes.** NEW protocol section: worker lifecycle doctrine (one-shot contract workers, no subagents, fresh-worker-per-round default, steering escape hatch for small fix + expensive in-flight state — applied on WIKI-261 rebase). Parked: store-pruning P2, worktree-debris P3 (~95 stale, +4 new this arc), artifact-rendering arc, WIKI-233 reds triage, WIKI-228/229, new todo P2/P3 UI ideas (focus mode, turn nav, sticky header, peek-hover, breadcrumbs, composer menu).
- **PHOEBE (orch `phoebe-dev`) — v3 ladder arc, HENRY WENT HANDS-ON 2026-08-06 afternoon:**
    ...
    - **PHO-15330 monitoring REDESIGNED (Henry 2026-08-06 eve): v2 sandbox-guard design (#13598, closed) replaced by one-shot polling tool `start_monitor(tool_name, params, wake_states, polling_interval_s, timeout_s)`** — host polls a registry-flagged read-only tool, wakes agent via #13530 poke machinery on state match or timeout; no cancel, no sandbox evaluator, no long-lived-process-API prerequisite. Impl PR #13606 onto foundation, round 2 in flight (r1 NEEDS-FIXES: lease fencing, durable delivery sweep + 5 med + Devin threads). **MERGE-ORDER GATE (Henry): #13606 strictly AFTER #13505 lands on main** (comment on PR). PHO-15253/15254 tickets canceled (PRs closed). Buildkite phoebe-ci-gate is new (#13599) and flaky on stacked PRs — GH Actions authoritative.
    ...
    - Rungs after foundation: control #13538, bash door #13416 (stacked, gates-complete). #13413 routing flags + #13414 (PHO-15254 typed people read libs) both CLOSED (Henry 2026-08-06 eve, superseded by ladder / query tool on main; resurrect 13414 only with a real deterministic-code consumer). #13408 closes when ladder covers it. #13193 parity harness owes rebase post-carve. #13540 reads rung: Henry's call.
