...
- PR [#302](https://github.com/hwang2409/wiki/pull/302) merged as `baa0a40d`.
- The combined #300/#301/#302 native build was staged, then removed with `wiki-cpu-fix` at Henry's request. A new app built from `origin/main` `897317cf` launched at 15:24 EDT on 2026-09-24.
- After the 15:44 EDT restart on `b33e9d67`, supervisor PID 2293 reached ~79% CPU at 15:52. `metadata.sqlite3` shows a parity pass for run `a4f51d8b-5d80-596f-8219-edb4b3fe100b` from 15:50:53 to 15:55:21 EDT. The run had 10,988 raw events; the pass recorded 38,609 mismatch rows (24,285 raw-prefix; 13,585 final; 739 other), then marked backfill complete with `matched=false`. CPU fell to ~4-9% after completion. Thus #302 bounded checkpoint count but did not bound per-mismatch SQLite writes or the cost of five large prefix replays.
- The same supervisor startup still calls synchronous `reconcile_archive_edges()` in `Supervisor.__init__`; the archive held 4,044 session dirs / 60 GB. This may explain part of the pre-backfill CPU time, but the 15:52 spike directly overlaps the measured parity pass. A future fix should cap recorded mismatch detail and defer or make archive-edge reconciliation incremental.
