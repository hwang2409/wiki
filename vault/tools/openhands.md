...
## What makes the harness good

- **CodeAct action space**: agent emits executable Python/bash instead of JSON tool calls ("Executable Code Actions Elicit Better LLM Agents", Wang et al 2024). Composable (many ops per turn), exploits code pretraining, cuts turn count → less context burn.
- **Event stream = source of truth**: session is append-only Action/Observation stream; state folds over it. Resumable, replayable, trajectories double as eval artifacts, UI renders from same stream.
- **Observation shaping**: truncated shell output, error tails kept, linter feedback after edits — SWE-agent's ACI finding: interface quality moves scores as much as model choice.
- **Runtime isolation**: execution server inside Docker sandbox, agent talks over API — safe arbitrary exec, parallel sessions, resettable env.
- **Context condenser**: summarizes old events at window pressure (same thesis as Devin's compression + our hot.md rewrite pattern).
- **Eval-loop discipline**: SWE-bench harness in-repo; changes measured, not vibed. Harness choices alone swing scores double digits at fixed model.
