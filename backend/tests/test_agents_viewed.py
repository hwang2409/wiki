...
class AgentsViewedTests(unittest.TestCase):
    ...
    def _write_run(self, run_id: str, *, seq: int, updated_at: str) -> None:
    ...
    def _seed_worker(
        ...
        *,
        ...
        self._write_run(run_id, seq=seq, updated_at=updated_at)
    ...
    def test_agents_payload_uses_run_json_freshness(self) -> None:
        run_id = _new_run_id()
        self._seed_worker("WIKI-42", run_id=run_id, seq=7, updated_at=_iso(0))
        ...
        # First read after migration marker: pre-deploy runs count as viewed
        # at their current seq — no dot on first paint of existing sessions.
        self.assertEqual(worker["last_viewed_seq"], 7)

    def test_new_session_created_after_migration_shows_unread(self) -> None:
        # Migration runs first with zero runs → viewed store is empty.
        main.agents()
        # New session materializes AFTER migration.
        run_id = _new_run_id()
        self._seed_worker("WIKI-77", run_id=run_id, seq=3, updated_at=_iso(60))
    ...
    def test_migration_backfills_existing_runs_once(self) -> None:
        pre_run = _new_run_id()
        self._seed_worker("WIKI-88", run_id=pre_run, seq=5, updated_at=_iso(0))
        # First read migrates.
        main.agents()
        stored = json.loads(self.viewed_path.read_text(encoding="utf-8"))
        self.assertEqual(stored[pre_run]["seq"], 5)
        self.assertIn("_migrated_at", stored)
        # New run added after migration — must NOT be added to viewed store.
        new_run = _new_run_id()
        self._seed_worker("WIKI-89", run_id=new_run, seq=2, updated_at=_iso(120))
        main.agents()
        stored = json.loads(self.viewed_path.read_text(encoding="utf-8"))
        self.assertNotIn(new_run, stored)
